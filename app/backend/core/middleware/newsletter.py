...
class NewsLetterFormMiddleware:
    ...
    def process_view(self, request, *args, **kwargs):
        ...
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest' and 'is_generic_nl_form' in request.POST:
            ...
            if form.is_valid():
                ...
                if captcha:
                    ...
                    if nl.exists():
                        data = {'result': 'Warning', 'message': "This email has already been used."}
                        return JsonResponse(data)
                    form.save()
                    data = {'result': 'Success', 'message': "We hope you enjoy our newsletter."}
                    return JsonResponse(data)
                else:
